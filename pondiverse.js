...
const DEFAULT_STORE = {
  ...
  export function addPondiverseButton(
    ...
    button.addEventListener("click", async (e) => {
      ...
      // Make sure we get the creation data before opening the dialog
      const getCreation = window["getPondiverseCreation"];
      if (getCreation) {
        try {
          // If getCreation returns a promise, await it
          const creationData = await Promise.resolve(getCreation());
          // Store the resolved data for use in openPondiverseDialog
          window["resolvedCreationData"] = creationData;
        } catch (error) {
          console.error("Error getting creation data:", error);
        }
      }
  ...
  export function openPondiverseDialog(
    ...
    // Check if we have already resolved creation data from the button click handler
    if (window["resolvedCreationData"]) {
      const creation = window["resolvedCreationData"];
      ...
      // Try to resolve it now if it wasn't resolved during the button click
      Promise.resolve(getCreation()).then(creation => {
        /** @type {HTMLInputElement | null} */
        const hiddenInput = dialog.querySelector("input[name='data']");
        if (!hiddenInput) throw new Error("Hidden input not found.");
        ...
        console.error("Error resolving creation data:", error);
      });
    }
  }
